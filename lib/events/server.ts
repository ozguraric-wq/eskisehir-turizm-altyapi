...
const TTL=6*60*60*1000,RETRY=30*60*1000;
...
/** Allowlisted HTTPS only; redirects are rejected so feeds cannot become an arbitrary proxy. */
export async function fetchOfficial(url:string,source:EventSource,fetcher:typeof fetch=fetch){
 ...
 const response=await fetcher(safe,{redirect:'error',signal:AbortSignal.timeout(8000),headers:{Accept:'text/html,application/ld+json;q=0.9','User-Agent':'ETAHB-EventCalendar/1.0'}});
...
export async function refreshEventSources(env:SocialEnv,fetcher:typeof fetch=fetch){
 ...
 const due=results.filter(r=>r.lease_until<now&&(!r.attempted_at||now-Date.parse(r.attempted_at)>(r.state==='unavailable'?RETRY:TTL))).slice(0,4);
 await Promise.allSettled(due.map(async row=>{
  ...
  }catch{await env.DB!.prepare("UPDATE city_event_feeds SET attempted_at=?,state='unavailable',lease_until=0 WHERE id=?").bind(stamp,row.id).run();}
 }));
}
export async function handleEventApi(request:Request,env:SocialEnv,ctx:{waitUntil(p:Promise<unknown>):void},fetcher:typeof fetch=fetch){
 ...
 try{
  ...
  if(url.searchParams.get('refresh')==='1'){await refreshEventSources(env,fetcher);return json(await getEventCatalog(env));}
