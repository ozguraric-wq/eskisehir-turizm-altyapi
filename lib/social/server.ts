...
export async function postDto(env:SocialEnv,p:Row,identity:Identity|null,origin:string){
 ...
 const stats=await env.DB!.prepare('SELECT COALESCE(SUM(liked),0) likes,COALESCE(AVG(NULLIF(rating,0)),0) rating,COUNT(NULLIF(rating,0)) ratingCount FROM social_votes WHERE post_id=?').bind(p.id).first<Row>();
 const comments=await env.DB!.prepare("SELECT COUNT(*) n FROM social_comments WHERE post_id=? AND status='approved'").bind(p.id).first<Row>();
 ...
 return {id:p.id,author:{id:p.user_id,name:author?.public_name??'Gezgin',handle:author?.status==='approved'?author.handle:''},title:p.title,body:p.body,placeIds:JSON.parse(p.place_ids),districts:JSON.parse(p.districts),mode:p.mode,days:p.days,preferences,status:p.status,revision:p.revision,createdAt:p.created_at,likes:stats?.likes??0,rating:stats?.rating??0,ratingCount:stats?.ratingCount??0,commentCount:comments?.n??0,media:files.results.map(m=>({...m,url:origin+'/api/community/media/'+m.id})),mine:owner,myVote:vote?{liked:!!vote.liked,rating:vote.rating}:undefined};
}
export async function handleSocialApi(request:Request,env:SocialEnv,platform:'sites'|'standalone'='standalone',fetcher:typeof fetch=fetch):Promise<Response>{
 ...
 try {
  if(path==='/status'&&method==='GET')return json({ready:configured(env),mediaReady:!!env.BUCKET,providers:providers(env),knowledgeAI:!!env.OPENAI_API_KEY,moderation:env.OPENAI_API_KEY?'ai-and-human-review':'human-review'});
  ...
  if(path==='/posts'&&method==='GET'){
   ...
   const sort=order==='popular'?"(SELECT COALESCE(SUM(liked),0) FROM social_votes v WHERE v.post_id=p.id) DESC,p.created_at DESC":'p.created_at DESC';
   const rows=await env.DB!.prepare(`SELECT p.* FROM social_posts p WHERE ${mine?"p.user_id=? AND p.status!='deleted'":"p.status='approved' AND ? IS NOT NULL"} AND (?='' OR EXISTS (SELECT 1 FROM json_each(p.districts) WHERE value=?)) AND (?='' OR p.mode=?) ORDER BY ${sort} LIMIT 12 OFFSET ?`).bind(mine?identity!.id:'public',district,district,mode,mode,page*12).all<Row>();
   return json({posts:await Promise.all(rows.results.map(r=>postDto(env,r,identity,url.origin))),hasMore:rows.results.length===12});
  }
  ...
  if(match){const [,id,action]=match;const post=await env.DB!.prepare('SELECT * FROM social_posts WHERE id=?').bind(id).first<Row>();if(!post||post.status==='deleted'||post.status!=='approved'&&post.user_id!==identity?.id&&!identity?.admin)throw new ApiError('not_found',404);
   ...
   if(action==='comments'&&method==='GET'){const rows=await env.DB!.prepare("SELECT c.*,p.public_name FROM social_comments c JOIN social_profiles p ON p.id=c.user_id WHERE c.post_id=? AND (c.status='approved' OR (c.user_id=? AND c.status!='deleted')) ORDER BY c.created_at DESC LIMIT 40").bind(id,identity?.id??'').all<Row>();return json({comments:rows.results.map(c=>({id:c.id,postId:id,author:{id:c.user_id,name:c.public_name},body:c.body,status:c.status,createdAt:c.created_at,mine:c.user_id===identity?.id}))});}
