...
import { Compass,Route,Bookmark,Building2,WifiOff,ScanLine,Globe2,ArrowLeft,Check,X } from "lucide-react";
...
export function MobileShell(){
  ...
  const c=mobileCopy(locale),q=qrCopy(locale),base=locale==="tr"?"":`/${locale}`;
  ...
  const tabs=[{href:base||"/",label:q.home,Icon:Compass},{href:`${base}/rotani-olustur`,label:q.route,Icon:Route},{href:`${base}/qr`,label:q.scan,Icon:ScanLine,scan:true},{href:`${base}/gezilerim`,label:q.trips,Icon:Bookmark},{href:`${base}/birlik`,label:q.union,Icon:Building2}];
  function switchLanguage(lang:Locale){const leaf=pathname.replace(/^\/(en|de|fr|ar)(?=\/|$)/,"");const target=["/qr","/birlik","/rotani-olustur","/gezilerim","/lezzet-ve-miras"].find(path=>leaf.replace(/\/$/,"")===path)??"";setLanguageOpen(false);router.push(`${lang==="tr"?"":`/${lang}`}${target||"/"}${target?location.hash:""}`);}
