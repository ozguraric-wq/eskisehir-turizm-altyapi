...
import {itemText} from "@/lib/routing/exports";
...
export function SavedTrips({locale="tr"}:{locale?:Locale}) {
  ...
  const title=(item:ScheduleItem)=>item.kind==="event"?item.event!.title:item.kind==="visit"?placeById[item.id].name:item.kind==="return"?rc.return:`${item.id.endsWith("dinner")?rc.dinner:rc.lunch} · ${foodAreas.find(f=>f.zone===item.zone)?.name??zoneNames[item.zone]}`;
  ...
  return <main id="ana-icerik" className="app-trips site-shell" lang={locale} dir={locale==="ar"?"rtl":"ltr"}>
    ...
    {!trip?<>
      ...
      <ol className="app-trip-timeline">{day.items.filter(item=>item.kind!=="return").map(item=>{const key=stopKey(dayIndex,item.id),done=trip.completed.includes(key);return <li key={key} className={done?"is-complete":""}><button type="button" className="app-visit-toggle" aria-pressed={done} aria-label={`${c.mark}: ${title(item)}`} onClick={()=>persist(trips.map(t=>t.id===trip.id?toggleVisit(t,key):t))}>{done?<Check size={20}/>:item.kind==="meal"?<Utensils size={18}/>:<MapPin size={18}/>}</button><div><span>{clock(item.start)}–{clock(item.end)}</span><h3>{title(item)}</h3><details><summary>{item.kind==="event"?eventCopy(locale).details:item.kind==="visit"?c.story:c.foodBreak}</summary><p>{item.kind==="event"?itemText(item,trip.preferences,locale).note:item.kind==="visit"?placeById[item.id].summary[locale]:trip.preferences.meal==="picnic"?rc.picnicNote:foodAreas.find(f=>f.zone===item.zone)?.[trip.preferences.meal==="vegetarian"?"vegetarian":"local"][locale]??rc.packedNote}</p>{item.kind==="event"&&<a href={item.event!.sourceUrl} target="_blank" rel="noreferrer">{eventCopy(locale).source}</a>}{item.kind==="visit"&&<a href={placeById[item.id].source} target="_blank" rel="noreferrer">{c.sources}</a>}</details>{discoveries[item.id]?.length>0&&<RouteDiscoveries items={discoveries[item.id]} locale={locale} area={`${zoneNames[item.zone]}, Eskişehir`}/>}</div></li>})}</ol>
